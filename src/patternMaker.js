...
function getPoints(pattern, direction, cornerIndex, x, y) {
	let bX = x * 3;
	let bY = y * 3;
	let corner = corners[cornerIndex];
	
	// no pointing inside
	if (cornerIndex === 0 && direction.x > 0 && direction.y > 0) return;
	if (cornerIndex === 1 && direction.x < 0 && direction.y > 0) return;
	if (cornerIndex === 2 && direction.x > 0 && direction.y < 0) return;
	if (cornerIndex === 3 && direction.x < 0 && direction.y < 0) return;

	const points = [
		[x + corner.x, y + corner.y],
		[x + corner.x + direction.x, y + corner.y + direction.y],
	].sort();
 ...
 */
export function getPattern(levelCount) {
	...
	for (let x = 0; x < cols; x++) {
		for (let y = 0; y < rows; y++) {
			...
			for (let i = 0; i < cornerCount; i++) {
				direction = directions[dirIndexes[directionIndex++]];
				getPoints(pattern, direction, i, x + ox, y);
...
export function getBounds(pattern) {
